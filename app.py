...
def put(data):
    file = "".join(data)
    r = requests.put(f"http://{hostname}:{port}/webhdfs/v1{hdfsPath}{file}?op=CREATE&overwrite=true&user.name={username}")
...
def append(data):
    ...
    r = requests.post(f"http://{hostname}:{port}/webhdfs/v1{hdfsPath}{hdfsfile}?op=APPEND&user.name={username}")
